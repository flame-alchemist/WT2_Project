...
def get_terms():
    print("called get terms")
    term = request.json['search_term']
    res = []
    cur = mysql.connection.cursor()
    cur.execute("select original_title, authors, image_url, original_publication_year, average_rating from books where original_title like '" + term + "%'");
    data = cur.fetchall()
...
@app.route('/api/v1/getList', methods=['POST'])
def get_list():
    ids = request.json['id']
    cur = mysql.connection.cursor()
    return_data = []
    for i in ids:
        # print(i)
        cur.execute('select original_title, authors, image_url, original_publication_year, average_rating  from books where id='+i)
        data = cur.fetchall()
        # print(data)
        tmp = {"title":data[0][0],"author":data[0][1],"url":data[0][2],"published":data[0][3],"rating":str(data[0][4])}
        return_data.append(tmp)
    print(return_data)
    return jsonify({"data":return_data}),200
...
def recommend_books():
    ...
    for book in books_list:
        if(book != name):
            book =  book.replace("'", "\'")
            cur.execute("select original_title, authors, image_url, original_publication_year, average_rating from books where original_title=" + "\"" + book + "\"")
            data = cur.fetchall()
            print(data)
            tmp = {"title":data[0][0],"author":data[0][1],"url":data[0][2],"published":data[0][3],"rating":str(data[0][4])}
            return_data.append(tmp)
